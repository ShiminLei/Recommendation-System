...
public class SearchItem extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 */
		/*
		 * JSONArray array = new JSONArray(); try { double lat =
		 * Double.parseDouble(request.getParameter("lat")); double lon =
		 * Double.parseDouble(request.getParameter("lon")); String keyword =
		 * request.getParameter("term");
		 * 
		 * TicketMasterAPI tmAPI = new TicketMasterAPI(); List<Item> items =
		 * tmAPI.search(lat, lon, keyword);
		 * 
		 * for (Item item : items) { JSONObject obj = item.toJSONObject();
		 * array.put(obj); } } catch (Exception e) { e.printStackTrace(); }
